...
<script type="text/javascript">
	$(document).ready(function(){
		...
		if(	operator == "3_checkedDesk"){
			//remove
			$("#dt_checkeddesk").removeClass("partnoClick-table pickingClick-table installClick-table checkedClick-table replaceClick-table limitClick-table");
			//add
			$("#dt_checkeddesk").addClass("checkedClick-table");
		}
		return false;
    });
</script>
...
$feederno1      = isset($_POST['feederno_chkd']) ? $_POST['feederno_chkd'] : '0';
...
$sql_zfeed = "select first 1 skip {$feederno}
				distinct zno 
				from checked_slctjob('{$get_jobno}')
				group by zno";
...
$sql 	= "select zfeeder, pol, pos, pos1, w_fs, p_sp, addrs, partnumber, point, demand,
			 ...
			 install_nik, install_name, install_date, install_time, addrs,
			 checked, checked_nik, checked_name, checked_date, checked_time
...
$sql_unchk_checked = "select count(*)
						from pn_checked('{$get_jobno}')
						where zfeeder containing '{$zfeed_ins}'
						and (checked is null or checked = '')
						and (partnumber <> null or partnumber <> '')";
$rs_unchk_checked	= $db->Execute($sql_unchk_checked);
$rs_unchk_checked->Close();
$tot_unchk_checked = trim($rs_unchk_checked->fields[0]);

$sql_sts 	= "select distinct sts_checked
				from checked_slctjob('{$get_jobno}')
				where zfeeder containing '{$zfeed_ins}'";
$rs_sts		= $db->Execute($sql_sts);
$sts	= $rs_sts->fields[0];
if 		($sts == '1'){ $stschk = '<font style="color: yellow; background-color: red"><b>STATUS : PROCESS</b></font>'; }
else if ($sts == '2'){ $stschk = '<font style="color: green;"><b>STATUS : CLEAR</b></font>'; }
else if ($sts == '3'){ $stschk = '<font style="color: red;"><b>STATUS : PASSED</b></font>'; }
else if ($sts == '4'){ $stschk = '<font style="color: red;"><b>STATUS : UNCLEAR</b></font><br>&nbsp;<br><font style="color: navy;">Please Call Administator,<br>If you want to start this Z-Feeder.</font>'; }
else if ($sts == '5'){ $stschk = '<font style="color: red;">STATUS : UNCLEAR</b></font><br>&nbsp;<br><font style="color: navy;">Please Call Administator,<br>&nbsp;<br>If you want to start this Z-Feeder.</font>'; }

if($exist == 0){
    ?>
        <h4 class="warning" align="center"><?=$stschk?></h4>
    <?php
}
else{
	echo'<input type="hidden" id="uncheck" value="'.$tot_unchk_checked.'" />';
	//echo'<input type="hidden" id="totpart" value="'.$totpart.'" />';
    echo'<table id="dt_checkeddesk" align="center" class="table table-stripse col-xs-12 col-sm-12 col-md-12 col-lg-12">';
    echo'<thead>';
        echo'<th>NO</th>';
        echo'<th>Z-FEEDER</th>';
        echo'<th>POL</th>';
    ...
    while(!$rs->EOF){
		...
		$addrs			  = trim($rs->fields['19']);
		$checked		  = trim($rs->fields['20']);
		$checked_nik	  = trim($rs->fields['21']);
		$checked_name	  = trim($rs->fields['22']);
		$checked_date	  = trim($rs->fields['23']);
		$checked_time	  = trim($rs->fields['24']);
             ...
             echo'<tr style="background-color:#d3f8d3">';
                ...
                echo'<td data-content="DEMAND">'.$demand.'</td>';
                    ...
                    if($checked === "OK"){
						...
						/*echo'<i id="cancl_checked" class="fa fa-window-close fa-lg pull-right" style="color: gray;" onclick="cancelChecked('.$no.')"></i>';
                        ...
                        echo'<td data-content="CHECKED" style="color: red;"><b>--</b></td>';
                    }
		...
		elseif(trim($partnumber) == ""){
             echo'<tr style="background-color:#455a64; color: #fff;">';
                ...
                echo'<td data-content="DEMAND">'.$demand.'</td>';
                    ...
                    if($checked === "OK"){
                        echo'<td data-content="CHECKED" style="color:  #009688;"><b>'.$checked.'</b>';
						/*echo'<i id="cancl_checked" class="fa fa-ban fa-lg pull-right" style="color: red;" onclick="cancelChecked('.$no.')"></i>';
						echo'<input type="hidden" name="partno_cancel'.$no.'" id="partno_cancel'.$no.'" value="'.$partnumber.'" />
						<input type="hidden" name="zfeeder_cancel'.$no.'" id="zfeeder_cancel'.$no.'" value="'.$zfeeder.'" />
						<input type="hidden" name="pos_cancel'.$no.'" id="pos_cancel'.$no.'" value="'.$pos_val.'" />';*/
						echo'</td>';
            ...
            echo'<tr style="color:red">';
                ...
                echo'<td data-content="POINT">'.$addrs.'</td>';
                echo'<td data-content="DEMAND">'.$demand.'</td>';
				echo'<td data-content="CHECKED" style="color: green; font-size: 20px;"><b>'.$checked.'</b></td>';
				echo'<td data-content="KET">'.$ket.'</td>';
            echo'</tr>';
        }
       $rs->MoveNext();
