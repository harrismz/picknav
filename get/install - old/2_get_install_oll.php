...
$sql 	= "select zfeeder, pol, pos, pos1, w_fs, p_sp, addrs, partnumber,
			point, demand, install, ket, zfd_name, zfd_no, zfd_tray
...
if($exist == 0){
    ?>
        <h4 class="warning" align="center"><?=$stsinstal?></h4>
    <?php
}
else{
	echo'<table id="dt_installoll" align="center" class="table table-stripse col-xs-12 col-sm-12 col-md-12 col-lg-12">';
    echo'<thead>';
        echo'<th>NO</th>';
        echo'<th>Z-FEEDER</th>';
    ...
    while(!$rs->EOF){
		...
		elseif(trim($partnumber) == ""){
            ...
            echo'<tr style="color:red">';
                ...
                echo'<td data-content="POINT">'.$point.'</td>';
                echo'<td data-content="DEMAND">'.$demand.'</td>';
				echo'<td data-content="INSTALL" style="color: green;"><b>'.$install.'</b></td>';
				echo'<td data-content="KET">'.$ket.'</td>';
            echo'</tr>';
        }
       $rs->MoveNext();
