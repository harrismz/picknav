...
for($i=0; $i<$zfeeder_total; $i++){
	...
	$sql_chk_jobdetail_1   	= "select * from pn_picking_check('{$jobno}')
										where zfeeder containing '{$zfeed[$i]}'
	...
	if(!empty($jobdetail_sts_1)){
		...
		//cek unpicking
		$sql_unpicking_1 	= "select distinct count(*) from pn_picking_check('{$jobno}')
								where zfeeder containing '{$zfeed[$i]}'
									and (partnumber <> null or partnumber <> '')
									and (sts_opstart = '1' or sts_opstart is null)
		...
		//cek data yang LIMIT
		$sql_limit_1  		= " select distinct count(*) from pn_picking_check('{$jobno}')
								where zfeeder containing '{$zfeed[$i]}'
									and (partnumber <> null or partnumber <> '')
									and (sts_opstart = '1' or sts_opstart is null)
...
if ($i===$zfeeder_total){
	...
	$sql_fnsh_jobheader 		= "select count(*) from pn_picking_check('{$jobno}')
									where jobdate = '{$jdate}' 
										and (sts_opstart = '1' or sts_opstart is null)";
	...
	elseif($exist_fnsh_jobheader == 0){
		...
		$sql_fnsh_jobheader_3 		= "select count(*) from pn_picking_check('{$jobno}')
										where jobdate = '{$jdate}'
											and (sts_opstart = '4')";
